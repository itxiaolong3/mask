...
$pagesize=10;
$sql="select *  from " . tablename("mask_user") ." WHERE  nickname LIKE :name || user_tel LIKE :name || id LIKE :name and uniacid=:uniacid order by  level desc";
...
$total=pdo_fetchcolumn("select count(*) from " . tablename("mask_user") .
    " WHERE  nickname LIKE :name || user_tel LIKE :name || id LIKE :name and uniacid=:uniacid ",array(':uniacid'=>$_W['uniacid'],':name'=>$where));
